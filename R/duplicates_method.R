...
#'   )
#' }
setMethod("getDuplicates", "character", function(.Object, similarityMatrix, threshold=0.9, mc=FALSE, progress=TRUE, verbose=TRUE){
  sourceDir <- .Object
  ...
  if (progress == FALSE){
    ...
    docInfoList <- dirApply(f=.getDocInfoList, sourceDir=.Object, targetDir=NULL, mc=mc, verbose=verbose, progress=progress)
  }
...
setMethod("annotateDuplicates", "character", function(.Object, targetDir, duplicates=NULL, verbose=TRUE, mc=FALSE, progress=TRUE){
  if (is.null(duplicates)){
    ...
    similarityMatrix <- documentSimilarity(x=ngramMatrixWeighed, select=docsToCompareMatrix, weighting="tfidf", verbose=verbose, progress=progress, mc=mc)
  ...
  newAttributes <- lapply(
    ...
    function(x){
      ...
      if (x %in% duplicates$duplicateFilename){
        retval <- c(
          isDuplicate="TRUE",
          isDuplicateOf=paste(subset(duplicates, duplicateFilename==x)[,"filename"], collapse="|")
        )
      }
      retval
    }
