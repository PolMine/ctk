...
replaceInvalidCharacters <- function(txt=NULL, sourceDir=NULL, targetDir=NULL, xml=FALSE, filenames=NULL, mc=FALSE, progress=FALSE, verbose=TRUE){
  ...
  if (!is.null(txt)){
    ...
    dirApply(f=.replaceInTxtFile, sourceDir=sourceDir, targetDir=targetDir, mc=mc, progress=progress, verbose=verbose, filenames=filenames, param=list())
  } else if (xml == TRUE){
