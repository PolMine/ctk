...
The java.parameters need to be set before a JVM is initialized.
...
```{r}
noCores <- parallel::detectCores() - 1L
```
...
```{r}
pipeDir <- "~/Lab/tmp/testpipe"
```

```{r}
for (subdir in c("xml", "csv", "tok", "ndjson", "vrt")){
  neededDir <- file.path(pipeDir, subdir)
  if (!file.exists(neededDir)) dir.create(neededDir) 
}
```

```{r}
P <- Pipe$new(dir = pipeDir, threads = noCores)
```

```{r}
sourceDirs <- c(
  "~/Lab/gitlab/plprbttxt_tei",
  "~/Lab/gitlab/plprbtpdf_tei"
)
P$getFiles(sourceDir = sourceDirs, pattern = "xml", targetDir = "xml")
```
...
P$basetable[, id := 1:nrow(P$basetable)] # add column with chunks
...
cat(
  P$tokenstream[["word"]], sep = "\n",
  file = file.path(P$dir, "tok", "tokenstream.vrt")
)
oldPipe <- new("pipe", projectDir = "~/Lab/tmp/testpipe")
Sys.setenv("PATH_TREETAGGER" = "/opt/treetagger")
treetagger(oldPipe, sourceDir = "tok", targetDir = "vrt")
tt <- data.table::fread("~/Lab/tmp/testpipe/vrt/tokenstream.vrt", sep = "\t")
P$tokenstream[["lemma"]] <- c("DUMMY", tt[[3]])
# P$tokenstream[["pos2"]] <- c("DUMMY", tt[[2]])
```
